...
func SetLevelObstacles(level):
	...
	for _i in get_children():
		_i.position = Vector2(float(Random.next_int_range(SpacerOffset, int(screenRes.x - SpacerOffset))), float(Random.next_int_range(SpacerOffset, int(halfRes.y - SpacerOffset))))

	# Generate an extra obstacle
	var obstacle = DestructibleBlock.instance()
	# Generate the obstacle's position.
	obstacle.position.x = float(Random.next_int_range(SpacerOffset, int(screenRes.x -SpacerOffset)))
	obstacle.position.y = float(Random.next_int_range(SpacerOffset, int(screenRes.y -SpacerOffset)))
	obstacle.setup(Random.next_int_range(SpacerOffset, int(scaler * level + SpacerOffset)), Color.white)
